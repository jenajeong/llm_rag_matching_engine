...
- 입력에 초록, 본문 등 문장 형태의 설명이 하나라도 포함된 경우:
  → 반드시 "명시적 추출만" 수행한다.
  → 텍스트에 직접 언급된 내용만 추출하며, 재구성/추론/보완은 절대 금지한다.
  → 제목 기반 규칙은 이 경우 완전히 무시한다.

- 입력이 제목(title)만 존재하는 경우에만:
  → "제목 기반 최소 재구성"을 수행한다.

[제목 기반 재구성 규칙]
- 제목에 포함된 단어를 기반으로만 구성 (외부 지식 사용 금지)
- 과도한 일반화 금지 (예: "성능 향상" 단독 사용 금지)
- 생성된 엔티티 간의 최소 관계를 반드시 1개 이상 생성
- 관계는 "의미적 흐름"이 아닌 "구조적 연결"만 허용
- 최소 3개의 엔티티는 반드시 생성(target, problem, achievement)
...
[관계 생성 규칙]
- target → problem (문제 존재 시)
- problem → achievement (성과 존재 시)
- target → achievement (problem 없을 경우)

- relationship_description은 제목 단어 기반으로 간단히 구성
- relationship_keywords는 제목 핵심 단어 조합으로 생성
